...
from calculate_hours import iterate_semesters
...
def push_in_list(sheet, teacher_subj, semester, lection=False):
    ...
    while sheet.cell(row, T_COL).value != 'END':
        ...
        if teacher not in teacher_subj:
            if lection:
                teacher_subj.update({teacher: [sem + str(row) + L + str(
                    int(sheet.cell(row, 0).value))]})  # it points that it's a lection table's row
            elif 'Пр' in sheet.cell(row, 3).value:
                teacher_subj.update({teacher: [
                    sem + str(row) + P + str(int(sheet.cell(row, 0).value))]})  # it's points that there is a practic
            else:
                teacher_subj.update({teacher: [
                    sem + str(row) + W + str(int(sheet.cell(row, 0).value))]})  # it points that there is a lab work
            row += 1
            continue
        if lection:
